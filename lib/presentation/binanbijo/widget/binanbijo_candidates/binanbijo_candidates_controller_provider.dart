...
final binanbijoCandidatesControllerProvider = FutureProvider<List<BinanbijoCandidateModel>?>((ref) async {
  final _repository = ref.read(binanbijoApplicationProvider);
  final result = await _repository.getCandidateList();
  if(result == null) return null;
  final candidateList = result.candidates.map((candidate) => BinanbijoCandidateModel.from(candidate)).toList();
